...
os.environ["OPENAI_API_KEY"] = "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"
...
def extract_text_from_file(file_name: str, file_bytes: bytes) -> List[Document]:
    logger.info("Extracting text from file: %s (size: %d bytes)", file_name, len(file_bytes))
    ...
    try:
        if ext == ".pdf":
            ...
            logger.info("Extracting %d pages from PDF: %s", len(reader.pages), file_name)
            ...
            logger.info("Extracted text from DOCX: %s (length: %d)", file_name, len(text))
            if text.strip():
                docs.append(Document(page_content=text, metadata={"source": file_name}))
        elif ext in [".txt", ".csv"]:
            text = file_bytes.decode(errors="ignore")
            logger.info("Extracted text from %s file: %s (length: %d)", ext, file_name, len(text))
            docs.append(Document(page_content=text, metadata={"source": file_name}))
        elif ext in [".png", ".jpg", ".jpeg"]:
            logger.info("Processing image file: %s", file_name)
            ...
            try:
                ...
                logger.warning("Could not open image preview for %s: %s", file_name, str(e))
            ...
            try:
                ...
                if pil.getbbox() is None:
                    ...
                    logger.warning("Image is empty or all-black: %s", file_name)
                    ...
                    try:
                        logger.info("Getting image description for: %s", file_name)
                        description = multi_embed.get_image_description(tmp_path)
                        logger.info("Image description generated (length: %d)", len(description))
                    except Exception as e:
                        logger.error("Image description call failed for %s: %s", file_name, str(e))
                ...
                logger.error("Error processing image %s: %s", file_name, str(e), exc_info=True)
                docs.append(Document(page_content="", metadata={"source": file_name, "error": str(e)}))
        elif ext == ".zip":
            logger.info("Extracting contents from ZIP file: %s", file_name)
            with zipfile.ZipFile(io.BytesIO(file_bytes), 'r') as z:
                logger.info("ZIP contains %d files", len(z.namelist()))
        ...
        logger.error("Error extracting text from %s: %s", file_name, str(e), exc_info=True)
        docs.append(Document(page_content="", metadata={"source": file_name, "error": str(e)}))
    
    logger.info("Extracted %d documents from file: %s", len(docs), file_name)
    return docs
...
def chunk_documents(documents: List[Document], chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP) -> List[Document]:
    ...
    for doc in documents:
        ...
        logger.debug("Document '%s' split into %d chunks", doc.metadata.get("source", "unknown"), len(chunks))
...
def load_persisted_vectorstore():
    logger.info("Loading persisted vectorstore from: %s", CHROMA_DIR)
    if not os.path.exists(CHROMA_DIR):
        logger.warning("Vectorstore directory does not exist: %s", CHROMA_DIR)
        return None
    ...
    try:
        ...
        logger.error("Failed to load persisted vectorstore: %s", str(e), exc_info=True)
...
def validate_user_query(query: str, max_len: int = 1500):
    logger.info("Validating user query (length: %d)", len(query) if query else 0)
    if not query or not query.strip():
        logger.warning("Query validation failed: empty query")
        return False, "Empty query."
    if len(query) > max_len:
        logger.warning("Query validation failed: exceeds max length of %d", max_len)
        return False, f"Query too long (>{max_len} chars)."
    banned = ["make a bomb", "how to kill", "bypass security"]
    lowered = query.lower()
    for b in banned:
        if b in lowered:
            logger.warning("Query validation failed: banned keyword detected: '%s'", b)
            return False, "Query contains disallowed content."
    logger.info("Query validation passed")
    return True, ""

def check_pii_in_text(text: str):
    logger.debug("Checking for PII in text (length: %d)", len(text) if text else 0)
    hits = {}
    if not text:
        return hits
    for k, pat in PII_PATTERNS.items():
        ...
        if found:
            hits[k] = found
            logger.warning("PII detected - Type: %s, Count: %d", k, len(found))
    if not hits:
...
def redact_pii(text: str):
    if not text:
        return text
    logger.info("Redacting PII from text (original length: %d)", len(text))
    out = text
    redaction_count = 0
    ...
    if redaction_count > 0:
        logger.info("Redacted %d PII instances", redaction_count)
    return out

def enforce_output_schema(text: str, max_len: int = 4000):
    logger.info("Enforcing output schema on text (length: %d)", len(text) if text else 0)
    if not text:
        logger.warning("Output schema validation failed: empty text")
        return False, "Empty model output."
    if len(text) > max_len:
        logger.warning("Output schema validation failed: exceeds max length of %d", max_len)
        return False, "Model output too long."
    refuse_keywords = ["how to make a bomb", "instructions to kill"]
    lower = text.lower()
    for k in refuse_keywords:
        if k in lower:
            logger.warning("Output schema validation failed: refused keyword detected: '%s'", k)
...
def log_audit_entry(event_type: str, payload: dict):
    logger.info("Logging audit entry - Event: %s", event_type)
    ...
    try:
        ...
        logger.debug("Audit entry written to %s", AUDIT_LOG)
    except Exception as e:
        logger.error("Failed to write audit entry: %s", str(e), exc_info=True)
        with AUDIT_LOG.open("a", encoding="utf8") as f:
            f.write(json.dumps(entry) + "\n")

def display_safe_text(text: str):
    logger.info("Displaying safe text (original length: %d)", len(text) if text else 0)
    safe = redact_pii(text)
    if len(safe) > 10000:
        logger.warning("Text truncated from %d to 10000 characters", len(safe))
        safe = safe[:10000] + "\n\n...[truncated]"
...
# Streamlit UI
# ---------------------------
logger.info("Starting Streamlit UI - Enterprise RAG Assistant")
